...
def plot_selected_blobs(rgb, cand_list, show=True, out_path=None):
    vis = rgb.copy()
    for d in cand_list:
        r0,c0,r1,c1 = d["bbox"]
...
def preprocessing(bgr, denoise_ksize=5, denoise_sigma=1.0,
                        ...
                        canny_sigma=0.33, se_gap=5, se_close=50, min_area=4000, max_area=310000):
    # 0 — colour to LAB
...
def compute_seeds(
    ...
    *,
    ...
    img : (H, W, 3) or (H, W) ndarray
        Preprocessed image as uint8 / float in [0, 255] or [0, 1].
...
def compute_single_descriptor_interpolated(contour: np.ndarray, n_samples: int = 11):
    ...
    features_trans_inv = descriptor[1:]

    # invariance to scaling
    eps = 1e-8  # numerical stability
    scale = np.abs(features_trans_inv[0]) + eps
    features_scale_inv = features_trans_inv / scale

    # invariance to rotation
    features_inv = np.abs(features_scale_inv)
...
def color_descriptors(rgbs, masks: np.ndarray, *, bins = 8):
    ...
    for i in range(N):
        mstd = color_mean_std(rgbs[i], masks[i] > 0)
        hist = color_hist(rgbs[i], masks[i] > 0, bins=bins)
...
def concatenate_descriptors(
    ...
    """
    Concatenate multiple descriptor types into a full feature vector.

    Parameters
    ----------
    region_feats : (D1) ndarray
    fourier_descs : (D2) ndarray
        Can be complex; real and imaginary parts are concatenated.
    color_descs : (D3) ndarray or None
    color_hists : (D4) ndarray or None
    lbps : (D5) ndarray or None

    Returns
    -------
    features : (D_total) float ndarray
        Each row is the concatenated descriptor vector for one image.
    """
    region_feats_np = np.array(region_feats, dtype=float)
...
def iqr_interval(values, factor=1.5):
    """
    Median ± factor·IQR   (25–75 % inter-quartile range).
    Resistant to small mask inaccuracies and outliers.
    """
    v = np.asarray(values, dtype=float)
    q25, q75 = np.percentile(v, [25, 75])
    iqr = q75 - q25
    med = np.median(v)
    return med - factor*iqr, med + factor*iqr

def geometric_priors_from_descriptors(desc_list,
                                      area_margin=0.10,
                                      comp_margin=0.05,
                                      iqr_factor=1.5):
    """
    Parameters
    ----------
    desc_list   : list[dict] – each dict must contain keys
                  'area' and 'compactness'  (or 'compacity')
    area_margin : enlarge area bounds by ±10 % (to avoid FN)
    comp_margin : enlarge compactness bounds by ±0.05
    iqr_factor  : how wide the IQR band is (default 1.5)
    ...
    areas = [d["area"] for d in desc_list if "area" in d]
    compacitites = [d["compacity"] for d in desc_list]

    if not areas or not compacitites:
        raise ValueError("Some reference descriptors miss 'area' "
                         "or 'compactness/compacity' fields.")

    Amin, Amax = iqr_interval(areas,  iqr_factor)
    Cmin, Cmax = iqr_interval(compacitites,  iqr_factor)

    # widen a little, to be safe
    Amin *= (1 - area_margin)
    Amax *= (1 + area_margin)
    Cmin -= comp_margin
    Cmax += comp_margin

    return (Amin, Amax), (Cmin, Cmax)
...
def extract_features_from_blob(img, blob_mask):
    ...
    features = np.concatenate([
        [region["area"],
         ...
         region["elongation"]],
        region["hu_moments"],
        np.hstack([fourier.real, fourier.imag]),
        color_descr,
        color_histo,
        lbp_histo
    ])
...
class kNNClassifier:
    ...
    def __init__(self, k : int, VI, ood_threshold: float = None):
        """
        Args:
            k (int): The number of neighbors to consider for the classification
            features: (N, d) feature of the N train samples
            labels: (N,) labels for train samples
        """
        self.k = k
    ...
    def fit(self, train_x, train_y):
        ...
        Args:
            train_x: (N, d) The tensor of training features
            train_y: (N,) The tensor of training labels
        """
        features = train_x.copy()
        labels = train_y.copy()
    ...
    def predict(self, test_x):
        ...
        Args:
            test_x: (N x d) The tensor of test features

        Returns:
            preds: (N,) The tensor of class predictions {0, 1, ..., n_classes}
            ood_scores: (N,) The OoD score predictions
        """
        distances = cdist(test_x, self.features, metric='mahalanobis', VI=self.VI)
        idx  = np.argsort(distances, axis=1)[:, :self.k]
        knn_labels = self.labels[idx]

        # majority vote
        preds = np.array([np.bincount(row).argmax() for row in knn_labels])

        # OoD
        ood_scores = np.take_along_axis(distances, idx, axis=1).mean(axis=1)
        if self.ood_threshold is not None:
            preds[ood_scores > self.ood_threshold] = -1
        
        return preds, ood_scores
    
    def set_ood_threshold(self, val: float):
        """
        Sets a new threshold for OoD detection.

        Args:
            val (float): New threshold value.
        """
        self.ood_threshold = val


def find_best_k(ks, kNNClassifier: Callable, VI, train_x, train_y, val_x, val_y):
    """
    Find the best k among different ks based on the validation set
    ...
    for k in ks:
        print(f"----- K: {k} -----")
        kNN = kNNClassifier(k, VI)
        kNN.fit(train_x, train_y)
        preds, _ = kNN.predict(val_x)
...
def fit_knn(best_k, VI, train_x, train_y, val_x, val_y):
    """
    After identifying the best k, compute the OoD threshold such that 95% of validation samples are detected as a chocolate type
    """
    classifier = kNNClassifier(best_k, VI)
    classifier.fit(train_x, train_y)

    # Predict on validation set without OoD rejection and set threshold
    print("----- WITHOUT OOD REJECTION -----")
    _, val_y_ood_scores = classifier.predict(val_x)
    threshold_ood = np.percentile(val_y_ood_scores, 95)
    print(f"ood threshold: {threshold_ood}")
    classifier.set_ood_threshold(threshold_ood)

    # Predict again with OoD rejection
    print("----- WITH OOD REJECTION -----")
    preds, _ = classifier.predict(val_x)
    metrics = compute_metrics(preds, val_y)

    return classifier, metrics


def compute_metrics(preds, true_labels, ood_label: int = -1):
    """ 
    Compute classification metrics for kNN predictions including OoD rejection.

    Args:
        preds: (N,) predicted class labels (integers 0-12 or -1 for unknown).
        true_labels: (N,) ground truth labels (0-12 or -1 for unknown/OoD).
        ood_label (int): Label used for OoD detection (default = -1).

    Returns:
        metrics (dict): Dictionary with accuracy, precision, recall, F1-score.
    """
    # Filter out OoD samples for accuracy
    valid_mask = (true_labels != ood_label)
    accuracy = np.mean(preds[valid_mask] == true_labels[valid_mask])

    # All known and OoD labels
    all_labels = sorted(set(np.unique(true_labels)).union(set(np.unique(preds))))

    # Classification report (handles OoD too)
    report = classification_report(true_labels, preds, labels=all_labels, output_dict=True, zero_division=0)
    ...
    metrics = {
        ...
        "per_class": {label: report[str(label)] for label in all_labels if str(label) in report}
    }
...
########################  7) FINAL TESTING & SUBMISSION FILE CREATION  ############################

def create_submission(path_to_file, pred):
    sub = pd.read_csv(path_to_file)
    print("template shape:", sub.shape) # sanity-check
    assert len(sub) == len(pred), "row mismatch!!"

    sub.iloc[:, 1:] = pred.astype(int)

    # ---  Alternate: align by image name ---------------------------------
    # If you want to be extra-safe (or your order differs) do this instead:
    #sub = sub.set_index(sub.columns[0])                     # index = image id
    #pred_df = pd.DataFrame(y_test_pred.astype(int),
    #                        index=test_filenames,
    #                        columns=sub.columns)             # same 13 class names
    #sub.update(pred_df)                                     # broadcast counts
    #sub = sub.reset_index()
    # ----------------------------------------------------------------------
